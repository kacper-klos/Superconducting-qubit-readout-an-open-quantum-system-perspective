...
leakage_heatmap = ArbitraryLevelSystemAnalysis.plot_leakage_probability_on_gap(
    7,
    kron(1.0 .* (1:3 .== 2), 1.0 .* (1:7 .== 2)),
    0.0:0.001:200.0,
    range(0.0, 0.5, length = 80),
)
...
qubit_excited_probability = TwoLevelTransmonAnalysis.plot_excited_probability(
    7,
    kron(TwoLevelTransmon.excited, 1.0 .* (1:7 .== 1)),
    0.0:0.001:200.0,
)
savefig(
    qubit_excited_probability,
    normpath(joinpath(IMAGES_PATH, "qubit_excited_probability.png")),
)
